...
@section('content')
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="post" id="myForm" action="{{ route('task.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-row">
                        ...
                        <select class="form-control select2" id="select-department" name="enduser_department">
                            <optgroup label="Select Department">
                                ...
                                @foreach ($department as $item)
                                    <option value="{{ $item->id }}" @if (old('enduser_department') == $item->id) selected @endif>
                                        {{ $item->department }}</option>
                                @endforeach
                            </optgroup>
                            <option value="OTHER" @if (old('enduser_department') == 'OTHER') selected @endif>OTHER</option>
                        </select>
                    </div>
                    <div class="form-group col-6" id="otherDepartmentInput" style="display: none;">
                        ...
                        <input type="text" class="uppercase form-control" id="other_department" name="other_department"
                            placeholder="Enter custom department" value="{{ old('other_department') }}">
                    </div>
